...
**The hash is now read as well as recorded.** Every link `match-amendments` writes carries one — 899 of 899 in the current dataset — and the command keeps a cache of the replies it buys, in `matches_cache.json` beside the dataset, as `extract-changes` does in `changes_cache.json`. A reply is reused only when the prompt hash recorded beside it is the one the build produces now, so a changed prompt buys a new reply instead of reusing an answer to a question nobody asked. Before that cache, the command re-queried the model in full on every run, which cost money and made the pipeline non-reproducible: the same commit over the same sources produced 899 links where it had produced 893 (#123). The key this ADR specifies was sufficient; only the reader was missing.
